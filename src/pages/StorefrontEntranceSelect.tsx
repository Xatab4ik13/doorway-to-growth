...
import { ENTRANCE_SUBCATEGORIES, entranceSubHref } from "@/lib/catalogRoutes";
...
export default function StorefrontEntranceSelect() {
  ...
  useDocumentMeta({
    title: site ? `Входные двери — ${site.name}` : "Входные двери — Brandoors",
    description: site
      ? `Входные двери Brandoors: стальные модели и двери с терморазрывом Термо — салон ${site.name}, ${site.city}`
      : "Входные двери Brandoors: стальные модели и двери с терморазрывом Термо",
  });
