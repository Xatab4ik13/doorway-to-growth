...
export function useStorefrontProducts(siteId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const PAGE = 1000;
      const select =
        "id, slug, name, rrp, sort_order, category_id, specifications, description, categories(name, slug), product_images(url, is_primary, sort_order, variant_key, glazing_key, edge_key, molding_key)";
