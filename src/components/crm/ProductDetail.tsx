...
const specs = [
  ...
  { label: "Толщина полотна", value: "40 мм" },
  { label: "Покрытие", value: "Экошпон" },
  { label: "Стекло", value: "Матовое" },
  { label: "Фурнитура", value: "В комплекте" },
];
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div>
              <div className="relative aspect-square rounded-2xl bg-muted flex items-center justify-center text-sm text-muted-foreground font-medium overflow-hidden">
                ...
                <button
                  ...
                  className="absolute left-2 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-card/80 text-foreground shadow-sm hover:bg-card active:scale-95 transition-all"
                >
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <button
                  onClick={() => setActiveImage(Math.min(images.length - 1, activeImage + 1))}
                  className="absolute right-2 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-card/80 text-foreground shadow-sm hover:bg-card active:scale-95 transition-all"
                >
              ...
              <div className="flex gap-2 mt-3">
                {images.map((_, i) => (
                  <button
                    ...
                    className={`h-14 w-14 rounded-xl bg-muted flex items-center justify-center text-[10px] text-muted-foreground transition-all active:scale-95 ${
                      activeImage === i ? "ring-2 ring-foreground" : "hover:ring-1 hover:ring-border"
                    }`}
                  >
                    {i + 1}
                  </button>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`h-2.5 w-2.5 rounded-full ${product.inStock ? "bg-success" : "bg-muted-foreground/30"}`} />
                <span className="text-xs text-muted-foreground">{product.inStock ? "В наличии" : "Нет в наличии"}</span>
              </div>

              <h2 className="text-xl font-semibold text-foreground mb-1">{product.name}</h2>
              <p className="text-2xl font-semibold tabular-nums text-foreground mb-6">
                {product.rrp.toLocaleString("ru-RU")} ₽
              </p>
              ...
              <div className="space-y-1.5 mb-6">
                {specs.map((s) => (
                  <div key={s.label} className="flex items-center justify-between py-1.5 text-sm border-b border-border last:border-0">
                  ...
                  <div key={c.label}>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {c.options.map((opt, i) => (
                        <button
                          key={opt}
                          className={`h-7 px-2.5 rounded-lg text-[11px] font-medium transition-colors active:scale-95 ${
                            i === 0
